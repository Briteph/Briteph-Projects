...
// grab mark as complete button
var completeButton = document.getElementById("mark-complete");

// grab planned time toggle switch container
var toggle = document.getElementById("is-planned");
var switchStatus = document.getElementById("switch-status");

// grab add new button and all the timefields
var newButtons = document.querySelectorAll(".add-new");

// grab activity selectors
var plannedActivitySelector = document.getElementById("planned-wrapper");
var unPlannedActivitySelector = document.getElementById("unplanned-wrapper");

//grab cancel buttons
var closeButtons = document.querySelectorAll(".close-modal");

// grab timelog form
var editForm = document.getElementById("edit-timesheet-form");

// grab inputs
var dateInput = document.getElementById("date");
var startTimeInput = document.getElementById("start-time");
var endTimeInput = document.getElementById("end-time");
// we already grabbed the is planned toggle above
var plannedActivityInput = document.getElementById("planned-activity");
var unPlannedActivityInput = document.getElementById("unplanned-activity");
var timelogIdInput = document.getElementById("timelogid");

// var clientWorkInput = document.getElementById('client-work');
// var clientWorkWrapper = document.getElementById('client-work-wrapper');

// grab save timesheet button
var saveButton = document.getElementById("save-timesheet");

// get popups
var popup = document.getElementById("edit-timelog-popup");
var approvePopup = document.getElementById("approve-popup");
// get delete button of popup
var deleteButton = document.getElementById("delete-timelog");
...
// get rating form buttons
var nextButtons = document.querySelectorAll(".save-rating");
var ratingForms = document.querySelectorAll(".rating-form");
...
// attach click event listeners to add new buttons
...
deleteConfirmButton.addEventListener("click", handleDeleteConfirmButtonClick);
...
expandRoles.addEventListener("click", handleExpandRolesButtonClick);

// attach toggle switch event listener
toggle.addEventListener("change", handleToggleChange);

// attach change event listener to activity selector, to show client work when needed
// plannedActivityInput.addEventListener('change', handlePlannedSelectChange);

// add form submission event listeners
editForm.addEventListener("submit", saveTimelog);
ratingForms.forEach((form) => {
  form.addEventListener("submit", handleRatingFormSubmission);
});

var plannedSlimSelect = new SlimSelect({
  select: "#planned-activity",
});

var unPlannedSlimSelect = new SlimSelect({
  select: "#unplanned-activity",
});

function handleExpandWeekendButtonClick(event) {
  event.preventDefault();
  const calendar = document.getElementById("timesheet-calendar");
  calendar.classList.contains("expanded")
...
function handleToggleChange(event) {
  ...
  plannedActivityInput.removeAttribute("required");
  unPlannedActivityInput.removeAttribute("required");
  // clientWorkInput.removeAttribute('required');

  if (isChecked) {
    switchStatus.textContent = event.currentTarget.dataset.on;
    unPlannedActivitySelector.classList.add("hidden");
    plannedActivitySelector.classList.remove("hidden");
    plannedActivityInput.required = true;

    /*      Client work input stuff
       if (plannedActivityInput.value === 'client'){
            clientWorkWrapper.classList.remove('hidden');
            clientWorkInput.required = true;
        } */
  } else {
    switchStatus.textContent = event.currentTarget.dataset.off;
    plannedActivitySelector.classList.add("hidden");
    unPlannedActivitySelector.classList.remove("hidden");
    unPlannedActivityInput.required = true;
    // if (!clientWorkWrapper.classList.contains('hidden')) clientWorkWrapper.classList.add('hidden');
  }
}

/* function handlePlannedSelectChange(event){
    if (toggle.checked && event.currentTarget.value === 'client'){
        clientWorkWrapper.classList.remove('hidden');
    }
    else {
        clientWorkWrapper.classList.add('hidden');
    }
} */

// handle "add new" button click
function handleAddNewClick(event) {
  // if it was clicked on an hourly slot, we need to get the index of the hour and the name of the day
  const parent = event.currentTarget.classList.contains("hour")
    ? event.currentTarget.parentElement
    : false;
  const clickedStartTime = parent
    ? [...parent.children].indexOf(event.currentTarget)
    : event.currentTarget.dataset.startTime;
  const clickedEndTime =
    clickedStartTime + 1 < 10
      ? `0${clickedStartTime + 1}:00`
      : `${clickedStartTime + 1}:00`;

  const timelog = {
    date: parent ? parent.dataset.date : event.currentTarget.dataset.date,
    startTime:
      clickedStartTime < 10
        ? `0${clickedStartTime}:00`
        : `${clickedStartTime}:00`,
    endTime: clickedEndTime == `24:00` ? `23:59` : clickedEndTime,
  };
  updatePopupContent(timelog);
  openPopup();
}

function handleEditClick(event) {
  const timelog = getTimelogById(event.currentTarget.id);
  updatePopupContent(timelog);
  openPopup();
}

function handleHelperNodeClick(event) {
  const data = event.currentTarget.dataset;
  const helperEndTime = parseInt(data.startTime.substring(0, 2)) + 1;
  const timelog = {
    date: data.date,
    startTime: data.startTime,
    endTime:
      helperEndTime < 10 ? `0${helperEndTime}:00` : `${helperEndTime}:00`,
  };
  updatePopupContent(timelog);
  openPopup();
}
...
function handleCancelDeleteButtonClick(event) {
  popup.classList.remove("deleting");
}
function handleDeleteConfirmButtonClick(event) {
  const timelog = getTimelogById(timelogIdInput.value);
  const buttonText = event.currentTarget.textContent;
  const form = document.getElementById("delete-confirmation");
  toggleFormProcessing(form);

  //fetch response
  fetch("/wp-admin/admin-ajax.php?action=delete_time_entry", {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
    },
    body: new URLSearchParams(timelog),
  })
    .then((response) => {
    ...
    .then((jsonResponse) => {
      ...
      if (jsonResponse.success) {
        const indexToDelete = state.indexOf(timelog);
        if (indexToDelete > -1) {
          state.splice(indexToDelete, 1);
          displayTimelogs();
          closePopup();

          // check if we need to delete subtask/subtimesheet
          if (
            timelog.sow &&
            state.filter((timelogInState) => timelogInState.sow === timelog.sow)
              .length < 1
          ) {
            // if yes remove it
            const taskToRemove = getTaskById(timelog.activity);
            // (timelog.activity);
            taskToRemove.partnerId = document.getElementById("partnerId").value;
            taskToRemove.partnerNonce =
              document.getElementById("partnerNonce").value;
            taskToRemove.editNonce = document.getElementById("editNonce").value;

            if (taskToRemove.type !== "role") {
              if (taskToRemove.existing) {
                fetch("/wp-admin/admin-ajax.php?action=delete_subtimesheet", {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/x-www-form-urlencoded",
                  },
                  body: new URLSearchParams(taskToRemove),
                })
                  .then((response) => {
                    return response.json();
                  })
                  .then((jsonResponse) => {
                    // toggleFormProcessing(form, buttonText);
                    //log response
                    if (jsonResponse.success) {
                      // remove it from state
                      taskState.splice(taskState.indexOf(taskToRemove), 1);
                      // rerender rating forms to DOM
                      renderRatingForms();
                    }
                  });
              } else {
...
function updatePopupContent(timelog = false) {
  if (timelog) {
    // hide delete button if we need to
    timelog.id
      ? deleteButton.classList.remove("hidden")
      : deleteButton.classList.add("hidden");
    // update inputs
    dateInput.value = timelog.date;
    startTimeInput.value = timelog.startTime;
    endTimeInput.value = timelog.endTime;

    // if timelog exists already, it'll have isPlanned values, so we need to set the inputs
    if (timelog.id) {
      // update hidden input since we are editing a timelog
      timelogIdInput.value = timelog.id;

      // check if timelog is planned
      toggle.checked = timelog.isPlanned;
      // need to dispatch the change event to change text and only show relevant activity selectors
      toggle.dispatchEvent(new Event("change"));

      // set activity selectors correctly, both in the input and SlimSelect
      if (timelog.isPlanned) {
        // if the timelog is planned, show planned activity selectors
        plannedActivityInput.value = timelog.activity;
        plannedSlimSelect.set(timelog.activity);
        unPlannedActivityInput.value = "";
        unPlannedSlimSelect.set("");
      } else {
        // if timelog is unplanned, show unplanned activity selectors
        plannedActivityInput.value = "";
        plannedSlimSelect.set("");
        unPlannedActivityInput.value = timelog.activity;
        unPlannedSlimSelect.set(timelog.activity);
      }
    } else {
      // update hidden input since we are entering new timelog
      timelogIdInput.value = "new";
    }
  }
}

function getTimelogById(id = false) {
  return id ? state.find((log) => log.id === id) : false;
}
...
function handleRatingFormSubmission(event) {
  event.preventDefault();
  // get values that we are going to use
  const form = event.currentTarget;
  const rating = form.rating.value;
  const slideAmount = 100 / ratingFormWrapper.childElementCount;
  const indexOfCurrentForm = [...ratingFormWrapper.children].indexOf(
    event.currentTarget
  );
  const translateAmount = (indexOfCurrentForm + 1) * slideAmount;
  ...
  if (form.nextElementSibling) {
    // get subtask from state
    const task = getTaskById(form.formId.value);
    task.rating = form.rating.value;
    task.comments = form.comments.value;
    task.editNonce = document.getElementById("editNonce").value;
    task.partnerNonce = document.getElementById("partnerNonce").value;
    task.partnerId = document.getElementById("partnerId").value;
    task.duration = task.id.startsWith("role")
      ? document.querySelector(`.progress-role[data-role=${task.id}`).dataset
          .minutes
      : false;
    task.durationNonce = task.id.startsWith("role")
      ? document.querySelector(`.progress-role[data-role=${task.id}`).dataset
          .nonce
      : false;
    task.ratingStartDate = document.getElementById("ratingStartDate").value;
    task.ratingEndDate = document.getElementById("ratingEndDate").value;
    task.unplannedDuration = getMinutesWorkedOnUnplannedSow(task.sow);
    task.roleDuration = getMinutesWorkedOnActivity(task.id);
    toggleFormProcessing(form);

    //fetch response
    fetch("/wp-admin/admin-ajax.php?action=save_ratingform", {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
      },
      ...
      .then((jsonResponse) => {
        ...
        if (jsonResponse.success) {
          ...
          ratingFormWrapper.style.transform = `translateX(-${translateAmount}%)`;
          task.existing = true;
          // task.subTaskId = jsonResponse.data.result.item_id;
          task.subTaskId = jsonResponse.data.result.Item_ID;
        }
      });
  } else {
    const task = {
      mainTaskId: document.getElementById("taskId").value,
      rating: form.rating.value,
      companyName: "Project Partners",
      comments: form.comments.value,
      editNonce: document.getElementById("editNonce").value,
      partnerNonce: document.getElementById("partnerNonce").value,
      partnerId: document.getElementById("partnerId").value,
    };
    toggleFormProcessing(form);

    //fetch response
    fetch("/wp-admin/admin-ajax.php?action=save_main_timesheet", {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
      },
...
function getMinutesWorkedOnActivity(activityId) {
  const minutesWorked = state
    .filter(({ activity }) => activity === activityId)
    .reduce((sum, timelog) => sum + timelog.duration, 0);
  return minutesWorked;
}

function getMinutesWorkedOnSow(sowId) {
  const minutesWorked = state
    .filter(({ sow }) => sow === sowId)
    .reduce((sum, timelog) => sum + timelog.duration, 0);
  return minutesWorked;
}
function getMinutesWorkedOnUnplannedSow(sowId) {
  const minutesWorked = state
    .filter(
      ({ sow, activity }) =>
        typeof activity === "string" &&
        activity.startsWith("quote") &&
        sow === sowId
    )
    .reduce((sum, timelog) => sum + timelog.duration, 0);
  return minutesWorked;
...
function saveTimelog(e) {
  ...
  const startDate = new Date(`${form.date.value}T${form.startTime.value}:00`);
  const endDate = new Date(`${form.date.value}T${form.endTime.value}:00`);

  const selectedOption = form.isPlanned.checked
    ? form.plannedActivity.selectedOptions[0]
    : form.unplannedActivity.selectedOptions[0];

  const timelog =
    form.timelogid.value == "new"
      ? { id: "new" }
      : getTimelogById(form.timelogid.value);
  const backupTimelog = { ...timelog };
  timelog.date = form.date.value;
  timelog.startTime = form.startTime.value;
  timelog.endTime = form.endTime.value;
  timelog.isPlanned = form.isPlanned.checked;
  timelog.activity = form.isPlanned.checked
    ? form.plannedActivity.value
    : form.unplannedActivity.value;
  timelog.activityTitle = selectedOption.innerText;
  timelog.startDate = startDate;
  timelog.endDate = endDate;
  timelog.duration = Math.round((endDate - startDate) / 60000);
  timelog.taskId = form.taskId.value;
  timelog.editNonce = form.editNonce.value;

  // if needed, get the quote ID - otherwise set to false
  timelog.quoteId = form.unplannedActivity.value.startsWith("quote")
    ? form.unplannedActivity.value.split("-")[1]
    : false;
  timelog.sow =
    timelog.isPlanned || timelog.quoteId
      ? document.querySelector(
          `.activity-select-wrapper option[value="${timelog.activity}"]`
        ).dataset.sow
      : false;

  const minutesWorkedOnActivity = getMinutesWorkedOnActivity(timelog.activity);
  timelog.roleDuration =
    timelog.id == "new"
      ? minutesWorkedOnActivity + timelog.duration
      : minutesWorkedOnActivity;
  toggleFormProcessing(form);

  //fetch response
  fetch("/wp-admin/admin-ajax.php?action=save_time_entry", {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
    },
    ...
    .then((jsonResponse) => {
      ...
      if (jsonResponse?.success) {
        console.log(jsonResponse.data);
        timelog.id = jsonResponse.data.timelog.data.ID;

        // check if we need to clone state first
        if (!getTimelogById(timelog.id)) {
          state.push(timelog);
        }
        displayTimelogs();
        closePopup();

        // need to check if timelog activity is billable
        // if(timelog.isPlanned || selectedOption.dataset.billable > 0){
        if (timelog.sow) {
          if (
            !taskState.find(
              (taskInState) =>
                taskInState.id === timelog.activity ||
                taskInState.sow === selectedOption.dataset.sow
            )
          ) {
            const task = {
              type: timelog.activity.split("-")[0],
              id: timelog.activity,
              companyName: selectedOption.dataset.company,
              sow: selectedOption.dataset.sow,
              mainTaskId: timelog.taskId,
              existing: false,
              subTaskId: false,
              comments: "",
              rating: "",
        ...
        Object.assign(timelog, backupTimelog);
      }
      addNotification(jsonResponse.success, jsonResponse.data.message);
    });
}
...
function addNotification(success, message) {
  ...
  document
    .getElementById("single-timesheet-container")
    .appendChild(responseMessage);
}
...
function displayTimelogs() {
  ...
  state.forEach((timelog) => {
    ...
    node.id = timelog.id;
    // console.log(node.id);
    node.className = timelog.isPlanned
      ? "single-timelog planned timelog-element"
      : "single-timelog unplanned timelog-element";

    if (!timelog.isPlanned && !typeof timelog.activity === "number") {
      node.classList.add("amber");
    }
    ...
    const startHour =
      timelog.startTime.substring(0, 2).replace(/^0+/, "") > 0
        ? parseInt(timelog.startTime.substring(0, 2).replace(/^0+/, ""))
        : 0;
    const startMinute = timelog.startTime.substring(3, 5).replace(/^0+/, "");
    const startHeight =
      document.querySelector(`.hour:nth-child(${startHour + 1})`).offsetTop +
      (hourHeight / 60) * startMinute;
    // (`Here's your node!! StartHour: ${startHour}, startMinute: ${startMinute}, and startHeight: ${startHeight}. Cheers!`);

    node.style.top = `${startHeight}px`;
    node.innerText = timelog.activityTitle;

    // add event listener to be able to edit
    node.addEventListener("click", handleEditClick);

    // get name of the day
    const dayName = timelog.startDate
      .toLocaleDateString("en-GB", { weekday: "long" })
      .toLowerCase();

    // if someone clicks right under the previous timelog (very common), we want to default to a seamless start time

    // calculate endTime
    const endHour = timelog.startTime.substring(0, 2).replace(/^0+/, "");
    const endMinute = timelog.endTime.substring(3, 5).replace(/^0+/, "");

    // insert node to DOM
    document
      .querySelector(`.day.${dayName}`)
      .insertAdjacentElement("beforeend", node);

    if (timelog.duration % 60 && endHour <= 23 && endMinute < 59) {
      // so if the timelog doesn't end on an exact hour, create a new clickable node element just under it
      const helperNode = document.createElement("div");
      helperNode.className = "timelog-helper timelog-element";
      helperNode.dataset.date = timelog.date;
      helperNode.dataset.startTime = timelog.endTime;
      // top position is calculated from the timelog's TOP position plus the height of the timelog
      helperNode.style.top = `${startHeight + nodeHeight}px`;
      helperNode.style.height = `${
        hourHeight - (timelog.duration % 60) * (hourHeight / 60) - 1
      }px`;
      helperNode.addEventListener("click", handleHelperNodeClick);

      // insert node to DOM
      document
        .querySelector(`.day.${dayName}`)
        .insertAdjacentElement("beforeend", helperNode);
    }
  });
  updateProgressBars();
}
...
function updateProgressBars() {
  ...
  function durationReducer(totalDurations, role) {
    ...
    if (role.activity.toString().startsWith("role")) {
      totalDurations[role.activity] =
        totalDurations[role.activity] + role.duration || role.duration;
    }
  ...
  if (Object.keys(currentStateOfRoles).length) {
    ...
    progressRoles.forEach((progressRole) => {
      ...
      if (currentStateOfRoles[progressRole.dataset.role]) {
        ...
        const percentage = Math.floor(
          (currentStateOfRoles[progressRole.dataset.role] /
            progressRole.dataset.minutes) *
            100
        );
        // update the DOM to reflect the changes
        progressRole.querySelector(".progress-percentage").textContent =
          percentage < 100 ? `${percentage}%` : `100%`;
        progressRole.dataset.percentage = percentage < 100 ? percentage : 100;

        // quick fix because CSS circle in not fully closing on 100% for some reason
        const dashArrayFirstOffset = percentage < 100 ? percentage : 104;

        //update progress bar
        progressRole
          .querySelector("svg circle.progress-ring-circle")
          .style.setProperty("--dashArrayFirstOffset", dashArrayFirstOffset);
        progressRole
          .querySelector("svg circle.progress-ring-circle")
          .style.setProperty("--dashArrayFirstBase", 60);
        // update strings
        ...
        const totalHours =
          currentStateOfRoles[progressRole.dataset.role] > 0
            ? Math.round(currentStateOfRoles[progressRole.dataset.role] / 6) /
              10
            : 0;
        progressRole.querySelector("span.hours-remaining").textContent =
          totalHours;
        ...
        progressRole
          .querySelector("svg circle.progress-ring-circle")
          .style.setProperty("--dashArrayFirstOffset", 0);
        progressRole.querySelector("span.hours-remaining").textContent =
          progressRole.querySelector("span.hours-remaining").dataset.hours;
      }
    });
    document.getElementById("weekly-total-hours").innerText = totalWeeklyHours;
  } else {
    ...
    document
      .querySelectorAll("svg circle.progress-ring-circle")
      .forEach((el) => el.style.setProperty("--dashArrayFirstOffset", 0));
    document
      .querySelectorAll("span.hours-remaining")
      .forEach((el) => (el.textContent = el.dataset.hours));
  }
}
...
if (
  ...
  barba.history.h.filter((e) => e.ns === "page-log-timesheet").length < 2
) {
  // target elements with the "draggable" class
  interact(".hour.add-new")
    .draggable({
...
function startDragListener(event) {
  ...
  let baseHeight = Math.floor(event.clientY - bounds.top);

  const roundToTheseHeights = [
    0,
    hourHeight * 0.25,
    hourHeight * 0.5,
    hourHeight * 0.75,
  ...
  let snapOffset = roundToTheseHeights.reduce(function (previous, current) {
    return Math.abs(current - baseHeight) < Math.abs(previous - baseHeight)
      ? current
      : previous;
  });
  let startMinute = (snapOffset / hourHeight) * 60;
  snapOffset = top + snapOffset;

  let startHour = [...parent.children].indexOf(target);
  ...
  // update inputs
  dateInput.value = parent.dataset.date;
  startTimeInput.value = `${startHour}:${startMinute}`;

  dragNode.id = "dragnode";
  dragNode.style.top = `${snapOffset}px`;
  dragNode.style.height = `${baseHeight}px`;
  dragNode.dataset.baseHeight = baseHeight;
  dragNode.dataset.startHour = startHour;
  dragNode.dataset.startMinute = startMinute;
  parent.appendChild(dragNode);
  document.documentElement.style.cursor = "grabbing";
}

function dragMoveListener(event) {
  var target = event.target;
  var hourHeight = document.querySelector(".hour").offsetHeight;
  var y = (parseFloat(target.getAttribute("data-y")) || 0) + event.dy;

  target.setAttribute("data-y", y);
  const dragNode = document.getElementById("dragnode");
  dragNode.style.height = `${y}px`;
}
...
function endDragListener(event) {
  ...
  // get start time
  const startHour = Number(dragNode.dataset.startHour);
  const startMinute = Number(dragNode.dataset.startMinute);

  // calculate duration
  const duration = Math.floor((dragNode.offsetHeight / hourHeight) * 100) / 100;
  const hour = Math.floor(duration);
  const minute = Math.round((duration - hour) * 60);

  let endMinute = minute + startMinute;
  let endHour = endMinute > 59 ? startHour + hour + 1 : startHour + hour;
  if (endMinute > 59) endMinute = endMinute % 60;
  const roundToMinutes = [0, 15, 30, 45, 60];
  endMinute = roundToMinutes.reduce(function (previous, current) {
    return Math.abs(current - endMinute) < Math.abs(previous - endMinute)
      ? current
      : previous;
  });
  if (endMinute > 59) {
  ...
  dragNode.remove();
  document
    .querySelectorAll(".hour.add-new")
    .forEach((el) => el.removeAttribute("data-y"));
}

if (typeof window.dragMoveListener !== "undefined") {
  window.dragMoveListener = dragMoveListener;
}

var resizeTimeout = false, // holder for timeout id
  resizeDelay = 250; // delay after event is "complete" to run callback

// window.resize callback function
function displayTimelogsAfterResize() {
  displayTimelogs();
  console.log("timelogs resized");
}
...
window.addEventListener("resize", function () {
  ...
  resizeTimeout = setTimeout(displayTimelogsAfterResize, resizeDelay);
});
